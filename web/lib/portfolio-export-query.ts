 ...
 *
 * The classification itself lives in `@/lib/agents/strategy-kind`, shared with
 * the public Universe summary — two surfaces describing where a book's
 * positions come from must not be able to disagree about the same agent. This
 * function adds only the per-kind knobs the pack renders.
 */
